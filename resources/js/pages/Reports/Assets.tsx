...
export default function Assets({ instituteAssets: instituteAssetsProp, institutes, blocks: initialBlocks, rooms: initialRooms, assetCategories, assets: initialAssets, regions, filters }: Props) {
  ...
  useEffect(() => {
    if (assetCategory) {
      ...
      // Reset to initial assets when no category is selected
      setAssets(initialAssets.filter(isValidItem));
      setAsset('');
    }
  }, [assetCategory]);
  ...
  useEffect(() => {
    ...
    fetch(`/reports/assets/institute-assets?${params.toString()}`)
      ...
      .catch((error) => {
        console.error('Error fetching assets after details toggle:', error);
        toast.error('Failed to fetch assets');
      });
  }, [details]);
  ...
  // Direct apply filters function (no debounce) for immediate execution
  const applyFilters = () => {
    const params = new URLSearchParams({
      search: search || '',
      institute_id: institute || '',
